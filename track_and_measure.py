...
class OutputData:
    def __init__(self, object_id, name, size, time, now):
        ...
        self.timeList = [time]
        self.timeSum = time
        self.timeLastSeen = now
    
    def add(self, size, time, now):
        self.sizeList.append(size)
        self.sizeMean = sum(self.sizeList)/len(self.sizeList)
        if self.timeLastSeen + time == now:
    ...
    def translate(self, dict_translate):
        self.nameJP = dict_translate.get(self.name)


dict_en2ja = {} # 英語をキーにして日本語を参照する辞書型リスト
with open('dict.csv', mode='r', encoding='shift-jis') as f:
    reader = csv.reader(f)
    dict_en2ja = {rows[0]:rows[1] for rows in reader}
...
fileHandler = logging.FileHandler(filename=f'{ROOT}/output/debug.log', encoding='utf-8')
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                if is_seg:
                    ...
                    # #############################################
                    # 追加ここから ################################ 0.3; 
                    sizeList_per_frame = [masks[i, :, :].sum() for i in range(len(masks))]
                    # 追加ここまで ################################
                    # #############################################
                else:
                    det[:, :4] = scale_boxes(im.shape[2:], det[:, :4], im0.shape).round()  # rescale boxes to im0 size
                ...
                if len(outputs[i]) > 0:
                    for j, (output) in enumerate(outputs[i]):
                        ...
                        if save_vid or save_crop or show_vid:  # Add bbox to image
                            ...
                            if id not in idList:
                                idList.append(id)
                                outputData.append(OutputData(id, names[c], sizeList_per_frame[i], processTime, nowTime))
                                outputData[-1].translate(dict_en2ja)
                            else:
                                for data in outputData:
                                    if data.objectID == id:
                                        data.add(sizeList_per_frame[i], processTime, nowTime)
                            # 追加ここまで ################################
                            # #############################################
